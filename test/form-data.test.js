...
test('validateFiles should validate all files', async () => {
  const files = [
  ...
  await assert.doesNotReject(
    validateFiles(files, 1024 * 1024) // 1MB limit
  );
});

test('validateFiles should reject list containing invalid files', async () => {
  const files = [
    { fieldName: 'file1', filePath: file1 },
...
test('createFormData should create FormData with single file', async () => {
  const files = [
...
test('createFormData should support multiple files', async () => {
  const files = [
    { fieldName: 'file1', filePath: file1 },
    { fieldName: 'file2', filePath: file2 }
  ];
...
test('createFormData should support custom fileName and contentType', async () => {
  const files = [
    {
      fieldName: 'image',
      filePath: imageFile,
...
test('createFormData should add formFields', async () => {
  const files = [
    { fieldName: 'avatar', filePath: file1 }
  ];
  const formFields = {
...
test('createFormData should add body field', async () => {
  const files = [
